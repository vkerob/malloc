...
void	initialize_data(t_data **data)
{
	...
	*data = (t_data *)mmap(NULL, ALLIGN_DATA + ALIGN_HEAP * 2 , PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	if (*data == MAP_FAILED)
		return ;
	...
	(*data)->small_heap = (t_heap *)((*data)->tiny_heap + ALIGN_HEAP);
...
void	initialize_large_heap(t_large_heap **new_large_heap, t_large_heap *large_heap_prev, size_t size)
{
	...
	(*new_large_heap) = mmap(NULL, ALIGN_LARGE_HEAP + size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
	...
	(*new_large_heap)->start = (void *)(*new_large_heap) + ALIGN_LARGE_HEAP;
...
static void	initialize_chunk(t_block *block, size_t size)
{
	...
	// add new chunk
	block->chunk = (void *)block + ALIGN_BLOCK;
	block->chunk->start = (void *)block->chunk + ALIGN_CHUNK;
	...
	int size_to_withdraw = (size_t)align_address((void *)block + ALIGN_BLOCK + ALIGN_CHUNK + size) - (size_t)block - ALIGN_BLOCK;
...
void	allocate(t_heap *heap, size_t size, size_t type_size)
{
	...
	block->free_size = type_size - ALIGN_BLOCK;
